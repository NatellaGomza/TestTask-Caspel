import type {DataType} from "../src/app/types";
...
export default [
    ...
    },
    {
        ...
        response: ({ query }) => {
            const { key } = query;
            // @ts-ignore
            const index = mockData.findIndex(item => item.key === key);

            if (index !== -1) {
                mockData.splice(index, 1);
                return { code: 0, message: 'Удалено' };
            }
            return { code: 1, message: 'Запись не найдена' };
        },
    },
    {
        url: '/api/record/edit',
        ...
        response: ({ query, body }) => {
            ...
            mockData[index] = { key, name, date, amount };
            return { code: 0, message: 'Запись обновлена' };
        },
    }
